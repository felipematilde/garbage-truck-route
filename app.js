...
var preDefinedBinList = [
  [-23.204641, -45.876315, 100, true], //1-HTO
  [-23.201778, -45.875185, 100, false], //2-H8
  [-23.200801, -45.874142, 100, true], //3-H23A
  [-23.200152, -45.873808, 100, true], //4-H22A
  [-23.199641, -45.872967, 100, false], //5-H17C
  [-23.199037, -45.872250, 100, true], //6-H17B
  [-23.196622, -45.872856, 100, false], //7-H25B
  [-23.196040, -45.871817, 100, false], //8-H25D
  [-23.195601, -45.868031, 100, false], //9-H9C
  [-23.196066, -45.867493, 100, false], //10-H9B
  [-23.196540, -45.867002, 100, true], //11-H9A
  [-23.200596, -45.865869, 100, true], //12-H30D
  [-23.204550, -45.864042, 100, true], //13-H12B
  [-23.204089, -45.865135, 100, true], //14-H11A
  [-23.202635, -45.869942, 100, false], //15-H21A
  [-23.202888, -45.871385, 100, false], //16-H18B
  [-23.200083, -45.869979, 100, false], //17-H27A
  [-23.203455, -45.870205, 100, true], //18-H18A
  [-23.205159, -45.868896, 100, true], //19-H10E
  [-23.203641, -45.868346, 100, true], //20-H30D
  [-23.199024, -45.870427, 100, true], //21-H27C
  [-23.197164, -45.868747, 100, false] //22-H25B
];
...
function findRouteForWaypoints(event){
    ...
    function findRouteDistanceAndTimeCallbackFunction(error, response) {
      ...
      console.log(`The distance is ${1000*response.route.distance}m`)
      console.log(`The route time is ${response.route.time}s`)
      alert(`The distance is ${1000*response.route.distance}m`)
      alert(`The route time is ${response.route.time}s`)
      return mapLeafLet;
    }
}
...
function removePredefinedNumberOfBinsRandomlyAndFindTheRoute(event){
  ...
  removePredefinedNumberOfBinsRandomlyFromBinWaypoints(nOfBinsToBeRemoved);
  findRouteForWaypoints(event);
}

function removePredefinedNumberOfBinsRandomlyFromBinWaypoints(nOfBinsToBeRemoved){
  var originalSizeOfWaypointsList = binWaypoints.length;
  if(nOfBinsToBeRemoved>originalSizeOfWaypointsList){
  ...
  while(listOfRemovedBins.length <nOfBinsToBeRemoved){
    var idxToBeRemoved = Math.floor(Math.random()*originalSizeOfWaypointsList);
    if(!listOfRemovedBins.includes(binWaypoints[idxToBeRemoved])){
      listOfRemovedBins.push(binWaypoints[idxToBeRemoved]);
    }
  }

  binWaypoints = binWaypoints.filter(function(value, index, arr){
